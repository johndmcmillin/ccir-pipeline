...
A medallion architecture data pipeline built on Azure Databricks that ingests real-time GPU instance pricing from AWS and Azure, normalizes it across cloud compute tiers, and produces a reference pricing index for GPU compute markets.

## Live Dashboard

[CCIR Index Dashboard](https://adb-7405613329444500.0.azuredatabricks.net/dashboardsv3/01f1199ee05911bd986fe4a8d25426c4/published?o=7405613329444500)
...
This pipeline is an attempt to answer that empirically, using publicly available pricing APIs and a structured lakehouse architecture that runs on a daily schedule and self-builds a time series of the index over time.
...
The full pipeline runs as a **scheduled Databricks Job** (`ccir-daily-index`) at 6:00 AM UTC daily. Each run appends a new snapshot to the Gold Delta table, building a time series of index values that can be queried historically via Delta Lake time travel.
...
- Azure prices Training and Inference meaningfully cheaper than AWS across all three tiers — the provider premium is not stable across tiers, which makes a single blended price meaningless as a reference rate

---

## Scheduling & Time Series

The pipeline runs daily via a Databricks Job with five sequential tasks:

```
ingest_aws → ingest_azure → bronze_autoloader → silver_transform → gold_index
```

Because the Gold table uses Delta Lake append mode, every daily run adds a new dated snapshot. This builds a historical time series of the index automatically — no additional code required. Index values at any historical point can be queried via Delta time travel:

```sql
SELECT * FROM gold_ccir_index TIMESTAMP AS OF '2026-03-06'
```
...
- **Databricks Jobs** — scheduled pipeline orchestration
- **Databricks SQL Dashboards** — live index visualization
